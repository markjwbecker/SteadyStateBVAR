forecast <- function (x, ci = 0.95, fcst_type = c("mean", "median"), growth_rate_idx = NULL, 
                      plot_idx = NULL, estimation = c("stan", "gibbs"), show_all = FALSE) 
{
  ...
  for (i in plot_idx) {
    ...
    if (!is.null(growth_rate_idx) && i %in% growth_rate_idx) {
      ...
      annual_hist <- ts(annual_hist, start = start(Y), 
                        frequency = freq)
      last_obs <- tail(smply, (freq - 1))
      all_fcst <- c(last_obs, fcst_m)
      annual_fcst <- rep(NA, H)
      for (t_h in 1:H) {
        annual_fcst[t_h] <- sum(all_fcst[t_h:(t_h + (freq - 
                                                       1))])
      }
      ...
      for (t_h in 1:H) {
        annual_lower[t_h] <- sum(all_lower[t_h:(t_h + 
                                                  (freq - 1))])
        annual_upper[t_h] <- sum(all_upper[t_h:(t_h + 
                                                  (freq - 1))])
      }
      ...
      if (isFALSE(show_all)) {
        ...
        plot.ts(annual_hist, main = paste(colnames(Y)[i], 
                                          "(annual)"), xlab = "Time", ylab = NULL, xlim = c(head(time_fore, 
                                                                                                 1) - (freq * 2), tail(time_fore, 1)), ylim = ylim, 
