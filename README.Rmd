...
For the posterior means of Psi, we see that they are well calibrated to be in the middle of the 'Longer run' ranges (as expected). Now lets forecast with a 95\% CI and use the mean of the predictive distribution as the point forecast
...
Now remember, for the unemployment rate, the 'Longer run' range was 3.8-4.5, with mid point 4.15, and for the Federal funds rate the Longer run range was 2.6-3.9 with mid point 3.25. Now the Median projection for the 'Longer run' values were 4.2 for the Unemployment rate and 3.0 for the Federal funds rate (and 2.0 for PCE inflation). As such, our forecasts at the last horizon should be close to the mid points and/or median projections.
...
We continue with the same data/model as in the previous section. Now suppose we are interested in the forecast of the Fed rate (FEDFUNDS) conditional on a scenario where the economy really heats up (inflation rises and unemployment decreases quickly).
