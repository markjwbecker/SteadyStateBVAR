...
Since the determinstic variables are i) a constant and ii) a dummy for if $t \leq 1992Q4$, we simply set $d_{t+1}'=\ldots=d_{t+H}'=\begin{pmatrix}1 & 0\end{pmatrix}$
...
```{r, fig.width=8, fig.height=10,eval=TRUE}
...
```{r, fig.width=10, fig.height=8, results='hide',eval=TRUE}
bvar_obj <- bvar(data = yt)
bp <- which(time(yt) == 1992.75)
dum_var <- c(rep(1,bp), rep(0,nrow(yt)-bp))
