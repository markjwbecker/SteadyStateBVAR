...
from $\epsilon_{T+h} \sim \textrm{N}(0, \textrm{I}_k)$, and then compute the shock to the VAR

$$
u^{(j)}_{T+h} = A^{-1} \Lambda^{0.5(j)}_{T+h} \epsilon^{(j)}_{T+h}
$$

After that calculate the forecast $\tilde{y}^{(j)}_{T+h}$ as per the usual way [see step 4 of Algorithm 4 in Karlsson (2013)].
