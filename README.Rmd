...
generate $\nu_{T+h}^{(j)}$ from $\nu_{T+h}\sim N(0,\Phi^{(j)})$, then compute $\ln \lambda_{T+h}^{(j)} = \gamma_{0}^{(j)} + \gamma_{1}^{(j)} \ln \lambda_{T+h-1}^{(j)} + \nu_{T+h}^{(j)}$. After that form $\Lambda_{T+h}^{(j)}$, and then compute the implied covariance matrix $\Sigma_{u,T+h}^{(j)} = A^{(j)-1} \Lambda_{T+h}^{(j)} (A^{(j)-1})'$.

Then generate the shock to the VAR $u_{T+h}^{(j)}$ from $u_{T+h} \sim N(0,\Sigma_{u,T+h}^{(j)})$ and then calculate the forecast $\tilde{y}_{T+h}^{(j)}$ as per the usual way [see step 4 of Algorithm 4 in Karlsson (2013)].

As I have had some problems implementing the forecasting routine directly in the Stan code, it is done in R/fit.R instead (for those who are interested). It does not make a difference however if it is done inside or outside of Stan, but I am working on fixing it.
...
As a last analysis, let us compare the steady-state BVAR with (SS-BVAR-SV) and without (SS-BVAR) stochastic volatility specification to see the difference. For the SS-BVAR we do the exact same setup, except now we have constant $\Sigma_u$ and use Jeffreys prior.
...
Very similar results for $\beta$ and $\Psi$. So now lets plot the forecasts.
