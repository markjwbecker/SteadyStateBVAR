...
Start from (the estimated) $\ln \lambda_{T}^{(j)}$, generate shocks $\nu_{T+1}$ from $N(0,\Phi^{(j)})$, compute $\ln \lambda_{T+1}^{(j)}$ and then form $\Lambda_{T+1}^{(j)}$. Then compute the implied covariance matrix $\Sigma_{u,t+1}^{(j)} = A^{-1 (j)} \Lambda_{t+1}^{(j)} (A^{-1 (j)})'$. Generate the shock to the VAR $u_t$ from $u_t \sim N(0,\Sigma_{u,t+1})$ and then compute $y_{t+1}^{j}$ as per the usual way [see step 4 in Algorithm 4 in Karlsson (2013)]. Then continue on from $\ln \lambda_{T+1}^{(j)}$ and continue the procedure until you reach $h=H$. This incorporates uncertainty associated with the time variation of $\Sigma_{u,t}$.
