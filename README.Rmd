...
Lets plot the forecast. Lets select a 95% prediction interval and the mean of the posterior as the point forecast. For the variables in annualized quarter on quarter growth rates, we transform the historical data and predictions to yearly growth rates.

```{r, fig.width=8, fig.height=8}
plot_forecast(stanfit, yt, ci=0.95, fcst_type="mean", growth_rate_idx=c(1,2,4,5))
```

Lets estimate the model with a Gibbs sampler instead. Everything is the same, except we now use the noninformative prior $\left|\Sigma \right|^{-(k+1)/2}$
