...
Clark (2011) extends the Steady-State BVAR(p) model (Villani, 2009) to allow the errors/innovations $u_t$ to have time varying covariance matrix $\Sigma_{u,t}$. The model is exactly the same as before on the surface. 
...
$$
u_t = A^{-1} \Lambda^{0.5}_t \epsilon_t, \ \ \ \ \ \epsilon_t \sim \textrm{N}(0, \textrm{I}_k)
$$

$$
\Lambda_t = \textrm{diag}(\lambda_{1,t},\dots,\lambda_{k,t})
$$
Here I follow Carriero, Clark and Marcellino (2024) which uses a more general version, with an AR(1) process with intercept for log volatilities instead of random walk as in Clark (2011) which in that case $\gamma_{0,i}=0  \ \ \forall i$  and $\gamma_{1,i}=1 \ \ \forall i$.

$$
\textrm{ln} (\lambda_{i,t}) = \gamma_{0,i} + \gamma_{1,i} \textrm{ln} (\lambda_{i,t-1}) + \nu_{i,t}
$$
For the innovations to the log volatilities, here in comparison with Clark (2011), log volatilities may be correlated, i.e. $\Phi$ is not necessarily diagonal.
