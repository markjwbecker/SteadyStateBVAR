...
After estimation, you can produce IRFs. The two available choices are: i) orthogonalized IRFs (Cholesky identification) and ii) generalized IRFs. To produce unconditional forecasts with the steady-state BVAR, we simply follow step 4 of Algorithm 4 of Karlsson (2013). To produce conditional forecasts, we follow Algorithm 3.3.1 in Dieppe, Legrand, and van Roye (2018). Please note that, for the moment, conditional forecasting is only available for the homoskedastic steady-state BVAR.

## Example 1 (Villani, 2009)

To estimate the model in Section 4.1 of Villani (2009) and perform impulse response analysis and forecasting, simply run the code in the [Appendix](#appendix). Here I go step-by-step and explain each part in considerable detail.

We are going to replicate the empirical analysis from Section 4.1 of Villani (2009). First, let us load the package and the data.
...
Finally for $\Sigma_u$ we will use the noninformative Jeffreys prior $\left|\Sigma_u \right|^{-(k+1)/2}$, as done in Villani (2009). We simply pass everything to the `priors()` function. Note here that the function automatically creates $\theta_\beta$ and $\Omega_\beta$.
...
One could perhaps argue for including a dummy variable for the period around 1970–1985, but we omit it and instead use only a constant as the deterministic variable.
...
This time we use the hyperparameters from Gustafsson and Villani (2025), i.e. overall tightness is set to $\lambda_1 = 0.27$, cross-equation tightness is set to $\lambda_2 = 0.43$ and the lag decay rate to $\lambda_3 = 0.76$. For the prior means on the first own lags, we set them to $0.6$ for $\Delta \pi_t$ and $0.9$ for $u_t$ and $r_t$. Note that the prior mean on the first own lag of inflation is set to $0.6$ instead of $0$ to reflect some degree of persistence in the series (even though it is a growth rate variable).
...
Let's put everything into the `priors()` function. Now instead of Jeffreys prior for $\Sigma_u$, we can use the uninformative inverse Wishart prior by setting 'Jeffrey=FALSE'.
...
Now let's forecast with a 68\% CI and use the median of the predictive distribution as the point forecast.
...
This section shows how to perform conditional forecasting. We will follow the implementation used in the BEAR toolbox (see Algorithm 3.3.1 in Dieppe, Legrand, and van Roye [2018]). Note that for the structural shocks, identification is based on the Cholesky factorisation.

We continue with the same data/model as in the previous section. Now suppose we are interested in the forecasts of inflation $\Delta \pi_t$ and the unemployment rate $u_t$ conditional on a scenario where the three-month Treasury bill rate $r_t$ (a proxy for the federal funds rate) rises quickly and unexpectedly, and then goes back down (this is just a toy example).

First we set up our conditions/scenarios, i.e., which variables, which horizons, and which values the variables will take during those horizons. Our conditions are that $r_t$ will follow a specified path at forecast horizons $h=1,\dots,H$.
...
Now we plot the conditional forecasts. The scenario is the blue line without intervals (since it is fixed) in the third plot. The red lines are the unconditional forecasts.
...
Clark (2011) extends the steady-state BVAR(p) model (Villani, 2009) by allowing the innovations $u_t$ to have a time-varying covariance matrix $\Sigma_{u,t}$. Here we further build on the model of Clark (2011) by following the setup in Section 1.1 ("*BVAR-SV Model*") in Carriero, Clark and Marcellino (2024). That is, instead of letting the log volatilities follow uncorrelated driftless random walks, we specify them as correlated (stationary) AR(1) processes. Carriero, Clark and Marcellino (2024) present the stochastic volatility specification for a conventional BVAR; here we extend it to the steady-state BVAR. However, the package also allows estimation of the Clark (2011) model (see the second subsection of this chapter).
...
contains the time-varying variances (log volatilities) of conditionally Gaussian shocks. The log volatilities follow AR(1) processes
...
where $\Phi$ is *not* diagonal and as such we allow the innovations to be correlated across variables. In the Clark (2011) model however, $\Phi$ is diagonal with variances $\phi_i$ for $i=1,\dots,k$.
...
The initial conditions ($\ln \lambda_0$) and initial observations ($y_0$) are both set to their unconditional means (or as we like to call them: steady states).
...
Here $a$ is a $k(k-1)/2$ vector that collects the free parameters in $A$ in row-major order, and $\ln \lambda_0$ are the time $t=0$ values (initial conditions) of $\ln \lambda_{i,t}$. Now we "cheat" and use highly informative priors centered on the true parameter values with low prior variances.
...
from $\epsilon_{T+h} \sim \mathrm{N}(0, \mathrm{I}_k)$. Now compute the shock to the VAR
...
We can also plot the estimates (posterior means) along with 95\% credible intervals of the log volatilities ($\ln \lambda_t$) in red. In grey, we plot the true unobserved/latent process. In blue, we plot the forecasts of the log volatilities along with a 95\% prediction interval.
...
```{r, echo=TRUE, results='hide', warning=FALSE, message=FALSE, fig.show='hide',eval=TRUE}
...
Again, we have $t=0,\dots,T=350$ observations where the last $50$ observations are left as hold-out data, and the parameters are
...
Since the log volatilities are Random Walks now, their steady states do not exist, and as such we specify some arbitrary initial conditions $\ln \lambda_0$
...
Note here that the inverse gamma $\mathrm{IG}(\alpha, \beta)$ distribution is the univariate version of the inverse Wishart distribution with $\alpha=m/2, \ \beta = V/2$.
...
Let's see if we managed to reasonably recover the true parameters. Remember again, that since $p=1$ we have $\beta'=\Pi_1$.
