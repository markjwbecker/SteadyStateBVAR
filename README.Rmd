...
where the autoregressive coefficient $\Pi_{\ell}^{(i,j)}$ is element $\left(i,j\right)$ of $\Pi_{\ell}$ for $\ell=1,\dots,p,$. As such, the Minnesota prior sets all prior means for the elements in $\beta$ to $0$, except for the elements that relate to the first own lags of the variables, which are set to $\kappa$.
...
To estimate the model in Section 4.1 in Villani (2009), and then forecast/do impulse response analysis, simply run the following code
...
```{r, fig.width=6, fig.height=6, warning=FALSE, message=FALSE,eval=FALSE}
...
Now if desired, the user can estimate the model with a Gibbs sampler instead.
...
The Gibbs estimation will give very similar results (as it should be). We will test if Gibbs and Stan truly give similar results in the following section.
...
```{r, fig.width=8, fig.height=8, warning=FALSE, message=FALSE,eval=FALSE}
...
We continue with the same data/model as in the previous section. Now suppose we are interested in the forecast of the Fed rate ($R_t$) conditional on a scenario where the economy really heats up (inflation [$\pi_t$]) rises and unemployment [$u_t$] decreases quickly).
