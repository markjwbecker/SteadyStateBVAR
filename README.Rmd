...
With this package, the user can estimate the steady-state BVAR(p) model (Villani, 2009). After estimation, the user can produce forecasts (unconditional and conditional) and impulse response functions (orthogonalized and generalized). The goal of `SteadyStateBVAR` is to use modern Bayesian tools (Stan) to: i) estimate the model as specified in the original paper, and ii) extend the model in different ways. Previously, extensions of the model seemed to be limited by what Mattias Villani had time to derive.
...
This package also allows for stochastic volatility (Random Walk or AR1 specifications), where we let the covariance matrix of the innovations vary over time (i.e. we have$\Sigma_{u,t}$, more details are found in `?bvar`).
