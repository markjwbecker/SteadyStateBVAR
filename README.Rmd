...
where $V_0$ is the scale matrix and $m_0\geq k+2$ are the degrees of freedom. An uninformative prior can be (and is in this package) specified by setting $V_0=(m_0-k-1)\hat{\Sigma}_u$ where $\hat{\Sigma}_u$ is the least squares estimate from the VAR($p$) (including the constant and dummy/trend variable if applicable), and $m_0=k+2$.

Later on, we introduce stochastic volatility and let the covariance matrix of the innovations vary over time such that we have a time-varying covariance matrix $\Sigma_{u,t}$.

After estimation, you can produce IRFs. The two available choices are i) Orthogonalized IRFs (Cholesky identification) or ii) Generalized IRFs. To produce unconditional forecasts with the steady-state BVAR, we simply follow step 4 of Algorithm 4 of Karlsson (2013). To produce conditional forecasts, we follow Algorithm 3.3.1 in Dieppe, Legrand, and van Roye (2018). Please note that for the moment, conditional forecasting is only possible for the homoskedastic steady-state BVAR.
